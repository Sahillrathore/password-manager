...
const data = [
    {
        flexD: "md:flex-row",
        title: "Protect what's important",
        para: "In today's digital age, safeguarding your personal information is paramount. With countless accounts and passwords to remember, a reliable password manager is essential. Protect what's important.",
        serial: "01.",
        taglines: ["lorem punka droop fk slot onprt", "lorem punka droop fk slot onprt", "lorem punka droop fk slot onprt"]
    },
    {
        flexD: "md:flex-row-reverse",
        title: "Generate Password",
        para: "Don't have a strong password in mind? don't worry our secure password generator got you covered generate a strong password according based on the inputs",
        serial: "02.",
        taglines: ["lorem punka droop fk slot onprt", "lorem punka droop fk slot onprt", "lorem punka droop fk slot onprt"],
        right: "-right-12",
        textD: "start "
    },
    {
        ...
        para: "Cross platform access for mobile, browser, and desktop apps.!",
        serial: "03.",
        taglines: ["lorem punka droop fk slot onprt", "lorem punka droop fk slot onprt", "lorem punka droop fk slot onprt"]
    },
            ...
            <h2 className='text-5xl font-bold text-center text-yellow-400 mb-4 mt-2'>Features</h2>
            {
                data.map((item, i) => (
                    <section className={`flex ${item.flexD} flex-col-reverse md:gap-36 gap-5 md:py-12 py-6 sm:px-12 px-4 justify-center items-center`} key={i}>
                        ...
                        <div className={`right relative md:w-fit w-full`}>
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 200 200" className={`absolute top-0 z-0 h-32 ${item.right ? "md:-right-12 right-40" : "right-36"}`}>
                            ...
                            <div className={`serial-no text-9xl font-semibold ${item.textD ? "md:text-start text-end" : "text-end"} md:pb-8 text-[#16379a] z-30 relative`}>
                                {item.serial}
                            </div>
